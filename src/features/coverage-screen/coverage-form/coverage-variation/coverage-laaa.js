...
const CoverageLaaa = ({ insurance, open }) => {
  ...
  return (
    <>
      ...
      <Box sx={sx.row}>
        ...
        <TextField name={APP_FORM.FIELDS_NAME.claimNumber} placeholder="Claim number" />
      </Box>
      <Box sx={sx.row}>
        <Typography sx={sx.label}>
          <CalendarIcon />
        ...
        <TextField name={APP_FORM.FIELDS_NAME.accidentDate} type={'date'} />
      </Box>
      <Collapse in={open}>
        <Box sx={sx.row}>
          <Typography sx={sx.label}>
